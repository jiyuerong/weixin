			...
			return this
					.each(function() {
						...
						var settings = $.extend({
							...
							selectButtonText : 'SELECT FILES', // The text to
																// use for the
																// browse button
							uploadButtonText : 'UPLOAD FILES',
							requireUploadButton : false,
							stopButtonText : 'STOP',
							requireStopButton : false,
							cancelButtonText : 'CANCEL',
							requireCancelButton : false,
						...
						if (flashInstalled) {
							...
							if (settings.requireUploadButton) {
								var $uploadButton = $('<div />', {
									'id' : settings.id + '-upload-button',
									'class' : 'uploadify-button '
											+ settings.buttonClass,
									'css' : {
										'float' : 'left',
							...
							if (settings.requireStopButton) {
								var $stopButton = $('<div />', {
									'id' : settings.id + '-stop-button',
									'class' : 'uploadify-button '
											+ settings.buttonClass,
									'css' : {
										'float' : 'left',
							...
							if (settings.requireCancelButton) {
								var $cancelButton = $('<div />', {
									'id' : settings.id + '-cancel-button',
									'class' : 'uploadify-button '
											+ settings.buttonClass,
									'css' : {
										'float' : 'left',
		...
		cancel : function(fileID, supressEvent) {
			...
			this.each(function() {
						...
						if (args[0]) {
							...
							if (args[0] == '*') {
								...
								$('#' + settings.queueID).find(
										...
										function() {
											...
											handlers.onCancel.call($this,
													swfuploadify.queueData.files[$(this).attr('id')]);
										});
								...
								for ( var n = 0; n < args.length; n++) {
									...
									handlers.onCancel.call($this,
											swfuploadify.queueData.files[args[n]]);
								}
							}
						} else {
							...
							handlers.onCancel.call($this,
									swfuploadify.queueData.files[$item.attr('id')]);
						}
					...
					.each(function() {
						...
						if (args.length === 1) {
							...
							case 'selectButtonText':
								$('#' + settings.id + '-select-button').find(
										'.uploadify-button-text').html(value);
								break;
							case 'uploadButtonText':
								$('#' + settings.id + '-upload-button').find(
										'.uploadify-button-text').html(value);
								break;
							case 'requireUploadButton':
								if (value) {
									$('#' + settings.id + '-upload-button')
											.show();
								} else {
									$('#' + settings.id + '-upload-button')
											.hide();
								...
								break;
							case 'stopButtonText':
								$('#' + settings.id + '-stop-button').find(
										'.uploadify-button-text').html(value);
								break;
							case 'requireStopButton':
								if (value) {
									$('#' + settings.id + '-stop-button')
											.show();
								} else {
									$('#' + settings.id + '-stop-button')
											.hide();
							...
							case 'cancelButtonText':
								$('#' + settings.id + '-cancel-button').find(
										'.uploadify-button-text').html(value);
								break;
							case 'requireCancelButton':
								if (value) {
									$('#' + settings.id + '-cancel-button')
											.show();
								} else {
									$('#' + settings.id + '-cancel-button')
											.hide();
	...
	var handlers = {
			...
			if ($.inArray('onUploadComplete', settings.overrideEvents) < 0) {
				...
				if (settings.isInput) {
					...
					case SWFUpload.FILE_STATUS.COMPLETE:
						if (settings.isInputSingle) {
							swfuploadify.queueData.queueSize = file.size;
							swfuploadify.queueData.queueLength = 1;
							$.each(swfuploadify.queueData.files, function(index, temp){
								if(temp.id != file.id){
									delete swfuploadify.queueData.files[temp.id];
								}
							});
						}
						setTimeout(
		...
		onUploadError : function(file, errorCode, errorMsg) {
			...
			case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
				alert('超过上传文件大小限制 (' + errorMsg + ').');
				errorString = '超出上传文件大小限制';
				break;
		...
		onCancel : function(file) {
			...
			if (settings.isInput) {
				var val = $("#" + settings.id + "-select-button input").val();
				if (val && val.length > 0) {
					...
					$("#" + settings.id + "-select-button input").val(
							arr.join());
				}
			}
			if (settings.onCancel) {
		...
		onUploadSuccess : function(file, data, response) {
			...
			if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
				...
				this.queueData.files[file.id].accessUrl = data;
				$('#' + file.id).find('.cancel').after(
						$('<div/>').addClass('download').append(
								$('<a/>').attr('href', data).attr('target',
										'_blank')));
			}
			if (settings.isInput) {
				if (!settings.isInputSingle) {
					var val = $("#" + settings.id + "-select-button input")
							.val();
					if (val && val.length > 0) {
						$("#" + settings.id + "-select-button input").val(
								val + ',' + file.name);
					...
					$("#" + settings.id + "-select-button input")
							.val(file.name);
				}
			}
